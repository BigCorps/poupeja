...
export const downloadCSV = (data: Transaction[]): void => {
  ...
  link.setAttribute('download', `vixus-relatorio-${new Date().toISOString().slice(0, 10)}.csv`);
...
export const downloadPDF = (data: Transaction[]): void => {
  try {
    ...
    // Set document title
    doc.setFontSize(20);
    doc.text('Relatório Financeiro - Vixus', 20, 20);
    ...
    // Save the PDF
    const fileName = `vixus-relatorio-${new Date().toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
