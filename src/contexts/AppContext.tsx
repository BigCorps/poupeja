...
// Importe a interface User diretamente do supabase se necessário, ou use a sua
...
      // dispatch({ type: 'SET_LOADING', payload: true }); // Removido para evitar piscar na UI
...
  // Ações de manipulação do estado
...
        // Define isLoading para true quando um usuário é encontrado
        if (session) {
          dispatch({ type: 'SET_USER', payload: session.user as unknown as User });
        } else {
          dispatch({ type: 'SET_USER', payload: null });
        }
      }
...
        // Chama todas as funções de busca de dados
        await Promise.all([
          getTransactions(),
          getCategories(),
          getGoals(),
